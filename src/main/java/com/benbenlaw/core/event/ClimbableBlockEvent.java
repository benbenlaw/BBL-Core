        ...
        if (entity instanceof Player player) {
            if (entity.horizontalCollision && isTouchingClimbable) {
                if (!player.isShiftKeyDown()) { // Only climb if not sneaking
                    Vec3 climbVec = new Vec3(movement.x, ModpackConfig.climbableBlockSpeed.get(), movement.z);
            ...
            //Down
                ...
                if (!player.isShiftKeyDown()) { // Only slow fall if not sneaking
                    Vec3 slowFallVec = new Vec3(movement.x, -ModpackConfig.climbableBlockSpeed.get(), movement.z);
