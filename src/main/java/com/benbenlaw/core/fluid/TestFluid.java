...
public class TestFluid {
    ...
    static {
        ...
        LIME_WATER = FLUIDS.register("eroding_water", (renderProperties) ->
                renderProperties.texture(Core.identifier("block/water_still"), Core.identifier("block/water_flow")).tint(0xFFCCCCFF));

        PINK_WATER = FLUIDS.register("pink_water", (renderProperties) ->
                renderProperties.texture(Core.identifier("block/molten_still"), Core.identifier("block/molten_flow")).tint(0xFFf72cff));
